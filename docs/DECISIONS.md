...
| D25 | Werks-User `admin` | **automatisch abschalten** · behalten | `adminUser="disable"` (Standard): Die Rolle `base` schaltet `admin` ab, sobald auf dem Gerät ein User aus `users` aktiv ist, nie vorher. Nach einem Secret-Push stößt der Manager dafür sofort einen Apply an. Die Rolle `manager` hinterlegt den Manager-Schlüssel auch für diese User, damit die `$cfm…`-Befehle ohne `admin` funktionieren. Ausnahme pro Gerät per Hostfile (`"keep"`). |
