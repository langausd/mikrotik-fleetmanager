...
| D26 | Manager-Funktionen | eine Datei · **Module + Lader** | `/file get` liest nur etwa 60 KB. Die Funktionen liegen daher in `lib/mgr-core.rsc`, `mgr-enroll.rsc`, `mgr-onboard.rsc` und `mgr-auto.rsc`. Die Rolle `manager` installiert jedes Modul aus dem Manifest als Skript `cfm-mgr-<modul>`, das Skript `cfm-mgr` lädt alle (Befehl im Terminal unverändert). Fehlen die Module im Manifest, bricht der Apply ab. |
