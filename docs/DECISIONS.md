...
## Verbesserungsplan, Punkt 4 (Runde 2026-09-13)

| # | Thema | Optionen | Entscheidung / Konsequenz |
|---|-------|----------|---------------------------|
| D34 | Prüfskript für RouterOS-Fallen | Sprache: **Python 3** · Bash/awk; CI: Lint + `:parse`, e2e manuell · Lint + `:parse` + e2e bei Push · **nur Lint** · keine; Hook: **Fehler blockieren** · nur warnen | `tools/rsc-check.py` zerlegt Skripte mit einem kleinen Tokenizer (Strings, Kommentare, Zeilenfortsetzung, Klammerebenen) und prüft die statisch erkennbaren Eigenheiten der Tabelle unten: `[` am Zeilenanfang, `\"` in direkten Funktionsargumenten, `:return` in `:onerror … in={}` (außer in Funktionsrümpfen), geräteabhängige Menüs in Slash-Schreibweise (außerhalb von Strings), Array-Literal ohne runde Klammern, `/import … verbose=yes`, unausgeglichene Klammern, Dateien über 60 KB (Hinweis ab 50 KB) und Dotfiles unter `cfm/work/`. Ausnahme per `# rsc-check: erlaubt <regel>`. Der Hook `tools/git-hooks/pre-commit` prüft den Index-Stand der gestagten Dateien; die GitHub Action prüft jeden Push samt Tests des Skripts. `:parse` und `e2e.sh` bleiben im lokalen Labor (CHR-Download, Laufzeit). |

## Umsetzungsentscheidungen (technisch begründet, nicht separat abgefragt)

* **Manager-Bootstrap auf leerer Config:** `bootstrap-manager.rsc` setzt das Gerät mit `clean="yes"` zuerst per `reset-configuration no-defaults=yes keep-users=yes run-after-reset=` zurück und läuft danach selbst weiter; `keep-users` erhält User, Passwörter und SSH-Schlüssel. Das run-after-reset-Skript läuft als Systemnutzer `*sys` ohne SSH-Schlüssel, und die Dienste starten erst nach ihm (siehe Eigenheiten). Deshalb richtet es nur `post`, den MGMT-Zugang und den User cfm mit einem Einmal-Passwort ein. Ein Scheduler lädt nach dem Hochfahren per SFTP mit diesem Passwort eine `.auto.rsc` an 127.0.0.1 hoch, die RouterOS als cfm ausführt; sie legt einen Scheduler an, der cfm gehört und den eigentlichen Bootstrap samt Selbst-Enroll ausführt. Danach bekommt cfm ein neues Zufallspasswort. Die Stufen erkennt die Datei an Markierungsdateien, Fortschritt und Fehler stehen im Log. So bleiben keine Reste einer Werks- oder CAPs-Config; auf einem bestehenden Manager wird der Reset verweigert. Verworfen: ein zweiter Import von Hand nach dem Reset (einfacher, aber nicht automatisch).
* **Lokale Site anlegen:** `tools/new-site.py` erzeugt ein Overlay aus den Beispieldaten samt Checkliste und ausgefülltem Bootstrap-Kopf; das Manager-Hostfile enthält nur den Uplink, weil Ports, die es auf dem Gerät nicht gibt, den Apply scheitern ließen. `tools/upload-seed.sh` lädt aus einem Overlay nur `*.rsc` (außer `bootstrap*.rsc`) und `hosts/`, `roles/`, `lib/`, `meta/`; Notizen, Checklisten und CSV-Listen bleiben lokal (vorher landeten sie in `work/` und damit in jedem Release).
...
| Ein Skript per `reset-configuration run-after-reset=` läuft als `*sys`: ohne SSH-Schlüssel (ssh-exec und SFTP-fetch mit Schlüssel: „authentication failure“), was es anlegt, gehört `*sys` (`owner` nicht setzbar). SSH, FTP und HTTP antworten erst, wenn das Skript beendet ist; ein Fehler bricht es ab (Log `system,error,critical`). CHR legt danach selbst wieder einen DHCP-Client auf ether1 an | Bootstrap-Stufe 2 kurz halten und mit `:onerror` absichern; Übergabe an cfm per SFTP-Upload einer `.auto.rsc` mit Einmal-Passwort nach dem Hochfahren (Manager-Bootstrap oben) |
| Verbindungen eines Geräts zu seiner eigenen Adresse laufen durch die input-Chain, kommen aber nicht über das Interface dieser Adresse herein (`in-interface-list` greift nicht) | Die Router-Firewall akzeptiert `src-address-type=local`; sonst scheitern auf einem Gerät mit Router- und Manager-Rolle Status-Abholung, Agent-Pull und Watchdog-Bestätigung an die eigene MGMT-IP, und der Watchdog rollt zurück |
